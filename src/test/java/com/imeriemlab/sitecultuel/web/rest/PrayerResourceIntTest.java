...
public class PrayerResourceIntTest {
    ...
    public void createPrayer() throws Exception {
        ...
        restPrayerMockMvc.perform(post("/api/prayers")
            ...
            .content(TestUtil.convertObjectToJsonBytes(prayer)))
    ...
    public void updatePrayer() throws Exception {
        ...
        // Update the prayer
        Prayer updatedPrayer = prayerRepository.findById(prayer.getId()).get();
        // Disconnect from session so that the updates on updatedPrayer are not directly saved in db
        em.detach(updatedPrayer);
        updatedPrayer
            .sobh(UPDATED_SOBH)
        ...
        restPrayerMockMvc.perform(put("/api/prayers")
            .contentType(TestUtil.APPLICATION_JSON_UTF8)
            .content(TestUtil.convertObjectToJsonBytes(updatedPrayer)))
            .andExpect(status().isOk());
